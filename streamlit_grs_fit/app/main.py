...
def main():
    ...
    if 'query' not in st.session_state:
        st.session_state.query = 'select '+\
            ...
            'from prod_publish.public.gcs_fit '+\
            'where gcs_cost > 0.01'  
    if 'df_data' not in st.session_state:
        st.session_state.df_data = load_data(st.session_state.query).set_index('JOB') 
    if submit:
        tab1, tab2, tab3 = st.tabs(['GRs', 'GCs', 'Table'])
        df_data = pd.DataFrame(st.session_state.df_data)
        df_data = df_data.loc[
                    ...
                    (min_total_costs < df_data.TOTAL_COST) & (df_data.TOTAL_COST < max_total_costs) &
...
def ModelAndScatterPlot(df, graphWidth, graphHeight, job_name, input_costs, known_grs_costs=None, known_gcs_costs=None):
    ...
    details = f'{job_name}: {feature_name} = ${input_costs:,.0f}, Predicted {target_name}: ${prediction:,.0f}\n'+\
                        ...
                        f'Function:  {target_name} = {fittedParameters[0]} * ln( {feature_name} + {fittedParameters[1]}) + {fittedParameters[2]}\n'+\
                        f'RMSE: {RMSE}\n'+\
